...
DEVICE = "cuda"
OUTDIR = "output/SDXL_ControlNet"
os.makedirs(OUTDIR, exist_ok=True)

# 1) SDXL + ControlNet(Canny)
cn = ControlNetModel.from_pretrained("diffusers/controlnet-canny-sdxl-1.0", torch_dtype=torch.float16)
pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
    "stabilityai/stable-diffusion-xl-base-1.0",
    controlnet=cn, torch_dtype=torch.float16, variant="fp16", use_safetensors=True
).to(DEVICE)

# 2) 조건맵: 스케치가 있으면 그걸 Canny로, 없으면 흰 캔버스 Canny(실질적으론 영향 적지만 선형화 유도)
canny = CannyDetector()
cond = canny(Image.new("RGB",(1024,1024),"white"), low_threshold=100, high_threshold=200)

# 3) 네가 Llama로 만든 ‘맥락 문장’들
briefs = {
